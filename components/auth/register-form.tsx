    ...
    startTransition(() => {
      register(values).then((data) => {
        ...
        if (data?.success) {
          ...
          // Opcional: Redirecionar para o login após X segundos ou limpar o form
          form.reset()
          setTimeout(() => {
             router.push("/login")
          }, 3000)
    ...
    <Card className="w-full shadow-md">
      <CardHeader className="space-y-1">
        ...
        <CardDescription className="text-center">
          Junte-se ao workspace e comece a construir.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Feedback de Sucesso */}
            {success && (
              <div className="bg-emerald-500/15 text-emerald-500 text-sm p-3 rounded-md font-medium">
                {success}
              </div>
            )}
